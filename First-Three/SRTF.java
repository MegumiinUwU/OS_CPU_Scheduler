...
public class SRTF implements Scheduler {
   ...
   public void run() throws Exception {
      ...
      while (i < processes.size() || currentProcess.getName() != null || !waitingProcesses.isEmpty()) {
         ...
         while (!waitingProcesses.isEmpty()) {
            ...
            tmpProcess.setWaitingTime(Math.max(tmpProcess.getWaitingTime(), 0) + 1);
            tmp.add(tmpProcess);
         }
         ...
         if(currentProcess.getName() != null && !waitingProcesses.isEmpty() && currentProcess.getCustomPriority() > waitingProcesses.peek().getCustomPriority()) {
            System.out.println("Process " + currentProcess.getName() + " has stopped at " + time);
            waitingProcesses.add(new Process(currentProcess));
            currentProcess = new Process(null, 0, 0);
         }
         ...
         if (currentProcess.getName() == null && !waitingProcesses.isEmpty()) {
            ...
            currentProcess.setWaitingTime(Math.max(currentProcess.getWaitingTime(), 0) + 1);
   ...
   public void plot() throws Exception {
      ...
      BufferedWriter bw = new BufferedWriter(new FileWriter("./shortestRemainingTimeFirst.txt"));
      for (Process p : stat) {
